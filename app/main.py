...
async def lifespan(app: FastAPI):
    ...
    # Quant stack schema migration (idempotent — safe to run every deploy)
    with engine.connect() as conn:
        for stmt in [
            ...
            "CREATE INDEX IF NOT EXISTS ix_shadow_meta ON shadow_signals (meta_should_trade, tb_label) WHERE meta_probability IS NOT NULL",
        ]:
