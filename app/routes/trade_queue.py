...
async def report_close(
    ...
    # Handle sim trades (ticket >= 999000) and OniAI virtual trades (800000-899999)
    is_sim = report.mt5_ticket >= 999000
    is_oniai = 800000 <= report.mt5_ticket < 900000

    if not trade and not is_sim and not is_oniai:
    ...
    if trade:
        ...
        trade.status = "ONIAI_CLOSED" if is_oniai else "CLOSED"
    ...
    sim_tag = " [SIM]" if is_sim else (" [OniAI]" if is_oniai else "")
    logger.info(
