...
async def _handle_health(telegram: TelegramBot):
    ...
    # Price providers (2-provider JIT: TwelveData + Finnhub)
    try:
        ...
        active = sum([td_ok, fh_ok])
        td_tag = "🟢" if td_ok else "🔴"
        fh_tag = "🟢" if fh_ok else "🔴"
        checks.append(f"{td_tag} TwelveData  {fh_tag} Finnhub  ({active}/2)")
    except:
...
async def _handle_providers(telegram: TelegramBot):
    """Show price provider health stats — 2-provider JIT architecture."""
    import os
    from app.services.server_simulator import get_provider_stats
    ...
    text = (
        "📡 <b>PRICE PROVIDERS (JIT 2-Provider)</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━\n\n"
    )

    for name in ["twelvedata", "finnhub"]:
        s = stats.get(name, {})
        ...
        if name == "twelvedata":
            ...
            extra = f" · {batches} batches"
        elif name == "finnhub":
            calls = s.get("individual_calls", 0)
            extra = f" · {calls} calls"

        text += f"{emoji} <b>{name}</b> · {success}✅ {fail}❌ · {rate}{extra}\n"
    ...
    text += (
        f"\n<b>API Keys:</b>\n"
        f"TwelveData {td_key} (8 req/min · batched)\n"
        f"Finnhub {fh_key} (60 req/min · fallback)\n"
    )
