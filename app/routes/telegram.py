...
async def _handle_status(telegram: TelegramBot, db: Session):
    ...
    await telegram._send_to_portfolio(text)
...
async def _handle_desk(telegram: TelegramBot, db: Session, desk_id: str):
    ...
    if desk_id not in DESKS:
        await telegram._send_to_portfolio(
...
async def _handle_kill(
    ...
    if scope == "ALL":
        ...
        if scope not in DESKS:
            await telegram._send_to_portfolio(f"❓ Unknown desk: {scope}")
            return
...
async def _handle_pause(
    ...
    if desk_id not in DESKS:
        await telegram._send_to_portfolio(f"❓ Unknown desk: {desk_id}")
        return
...
async def _handle_resume(telegram: TelegramBot, db: Session, desk_id: str):
    ...
    await telegram._send_to_portfolio(f"▶️ <b>{label}</b> resumed. Trading active.")
...
async def _handle_mlstats(telegram: TelegramBot, db: Session):
    ...
    await telegram._send_to_portfolio(text_msg)
...
async def _handle_diagnostics(telegram: TelegramBot):
    ...
    await diag.stop()
    await telegram._send_to_portfolio(report)
...
async def _handle_health(telegram: TelegramBot):
    ...
    # Price providers
    try:
        from app.services.price_service import _provider_stats
        active = sum(1 for s in _provider_stats.values() if s["success"] > 0)
        checks.append(f"🟢 Prices ({active}/6 active)" if active > 0 else "🔴 Prices (0 active)")
    except:
...
async def _handle_providers(telegram: TelegramBot):
    ...
    from app.services.price_service import PriceService, _provider_stats
    ...
    for name, stats in _provider_stats.items():
        ...
        if total == 0:
            ...
            if pct > 90:
                ...
                status = "🔴"
            rate = f" · {pct:.0f}% success"

        text += f"{status} <b>{name}</b> · {stats['success']}✅ {stats['fail']}❌{rate}\n"
    ...
    if os.getenv("ALPHA_VANTAGE_API_KEY"):
        keys.append("AlphaVantage ✅")
    else:
        keys.append("AlphaVantage ❌")
    keys.append("CoinGecko ✅ (free)")
    keys.append("Yahoo Finance ✅ (free)")
    keys.append("ExchangeRate ✅ (free)")
