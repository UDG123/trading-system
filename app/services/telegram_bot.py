...
class TelegramBot:
    ...
    def _format_timeframe(self, tf: str) -> str:
        """Format raw timeframe to display string."""
    ...
    async def notify_trade_entry(self, trade_params: Dict, decision: Dict):
        ...
        session = trade_params.get("session", "")
        ...
        tf_display = self._format_timeframe(timeframe)

        # Trend display
        trend = trade_params.get("trend", "UNKNOWN")
        trend_emoji = {
            "STRONG_UP": "🟢📈", "UP": "🟢↗️", "WEAK_UP": "🟡↗️",
            "STRONG_DOWN": "🔴📉", "DOWN": "🔴↘️", "WEAK_DOWN": "🟡↘️",
            "RANGING": "⚪↔️", "UNKNOWN": "❓",
        }.get(trend, "❓")
        trend_label = trend.replace("_", " ")

        # Direction vs trend alignment
        is_long = direction.upper() in ["LONG", "BUY"]
        trend_up = trend in ["STRONG_UP", "UP", "WEAK_UP"]
        trend_down = trend in ["STRONG_DOWN", "DOWN", "WEAK_DOWN"]
        if (is_long and trend_up) or (not is_long and trend_down):
            align = "✅ WITH TREND"
        elif (is_long and trend_down) or (not is_long and trend_up):
            align = "⚠️ COUNTER-TREND"
        else:
            align = "↔️ NEUTRAL"

        # Gold sub-strategy detection
        gold_sub = ""
        if desk == "DESK4_GOLD":
            tf_raw = str(timeframe).upper()
            if tf_raw in ["5", "5M", "M5"]:
                gold_sub = " ⚡ SCALP"
            elif tf_raw in ["60", "1H", "H1"]:
                gold_sub = " 📈 INTRA"
            elif tf_raw in ["240", "4H", "H4"]:
                gold_sub = " 🏛 SWING"
        ...
        try:
            ...
            if entry_f > 0:
                pip_size, pip_val = get_pip_info(str(symbol))
        ...
        if rsi_val is not None:
            rsi_line = f"📉 RSI       {rsi_val:.0f}\n"

        text = (
            f"📊 <b>TRADE SIGNAL</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{desk_emoji} <b>{desk_label}</b>{gold_sub} · {symbol} {arrow} <b>{direction}</b>\n"
            f"🕐 {tf_display} · {trend_emoji} {trend_label}\n"
            f"🧭 {align}\n\n"
            f"💲 Entry    {price}\n"
            f"🛑 SL       {sl}\n"
            f"🎯 TP1      {tp1}\n"
        ...
        # Portfolio summary
        summary = (
            f"{desk_emoji} <b>{desk_label}</b>{gold_sub} · {symbol} {arrow} {direction} "
            f"@ {price} · {tf_display} · {trend_emoji} {trend_label} · ${risk:.0f} risk"
        )
    ...
    async def notify_trade_exit(
        ...
        if "SRV_" in reason:
            source_tag = " 🖥️"
            clean_reason = reason.replace("SRV_", "")
        elif "MT5_" in reason:
            source_tag = " 📟"
            clean_reason = reason.replace("MT5_", "")
        elif "ONIAI_" in reason:
            source_tag = " 🤖"
            clean_reason = reason.replace("ONIAI_", "")
        else:
            clean_reason = reason

        text = (
            f"{emoji} <b>TRADE CLOSED — {result}</b>{source_tag}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    async def notify_oniai_signal(self, trade_params: Dict, decision: Dict):
        ...
        summary = (
            f"🤖 <b>OniAI</b> · {desk_label}{gold_sub} · {symbol} {arrow} {direction} "
            f"@ {price} · {tf_display} · ⚠️ {block_reason}"
        )
        await self._send_to_portfolio(summary)
    ...
    async def notify_signal_skip(
        ...
        text = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{desk_emoji} <b>{desk_label}</b>{gold_sub} · {symbol}\n"
            f"🕐 {tf_display}\n\n"
            f"📊 Score    {score}\n"
            f"📋 Reason   {reason[:150]}"
        )
