...
class DiagnosticsService:
    ...
    async def _check_price_providers(self) -> List[str]:
        """Check price provider health from stats."""
        issues = []
        try:
            ...
            for name, stats in _provider_stats.items():
                ...
                if fail_rate > 0.5 and total > 10:
                    issues.append(
                        f"🟡 Price provider {name}: {fail_rate*100:.0f}% failure rate "
                        f"({stats['fail']}/{total})"
                    )

                if fail_rate > 0.9 and total > 20:
                    issues.append(
                        f"🔴 Price provider {name} DOWN: {fail_rate*100:.0f}% failures"
                    )
    ...
    async def _send_diagnostic_alert(self, issues: List[str], repairs: List[str]):
        ...
        try:
            await self.telegram._send_to_portfolio(text)
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                active_providers = sum(
                    1 for s in _provider_stats.values()
                    if s["success"] > 0
                )
            ...
            text = (
                ...
                f"━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"📡 Price Providers   {active_providers}/5 active\n"
