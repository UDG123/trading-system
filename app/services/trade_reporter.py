...
class TradeReporter:
    ...
    def _pip_value(self, symbol: str) -> float:
        ...
        if "JPY" in sym:
            ...
            return 50.0
        elif any(x in sym for x in ["US30", "US100", "NAS", "SPX"]):
            return 1.0
    ...
    def _format_desk_report(
        ...
        # Split real trades vs OniAI virtual trades
        real_trades = [t for t in trades if not (t.close_reason or "").startswith("ONIAI")]
        oniai_trades = [t for t in trades if (t.close_reason or "").startswith("ONIAI")]
        ...
        if real_trades:
            text += "\n"
            text += self._format_trade_section(real_trades, desk_id, "EXECUTED")

        # OniAI section
        if oniai_trades:
            text += (
                f"\n\n🤖 <b>OniAI VIRTUAL TRADES</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━\n"
            )
    ...
    def _format_trade_section(
        ...
        for t in trades:
            ...
            oniai_tag = " 🤖" if "ONIAI" in reason else ""

            if pips >= 0:
                trade_lines.append(
                    f"🟩 {symbol} {arrow} {direction}    +{abs(pips):.0f}p{sim_tag}{oniai_tag}"
                )
                total_pips_won += pips
                wins += 1
            else:
                trade_lines.append(
                    f"🟥 {symbol} {arrow} {direction}    {pips:.0f}p{sim_tag}{oniai_tag}"
                )
    ...
    def _format_portfolio_report(
        ...
        # Separate real vs OniAI across all desks
        real_by_desk = {}
        oniai_by_desk = {}
        for desk_id, trades in all_trades.items():
            real_by_desk[desk_id] = [
                t for t in trades if not (t.close_reason or "").startswith("ONIAI")
            ]
            oniai_by_desk[desk_id] = [
                t for t in trades if (t.close_reason or "").startswith("ONIAI")
            ]
        ...
        # EXECUTED section
        text += self._format_portfolio_section(real_by_desk, "EXECUTED")
    ...
    def _get_closed_trades(
        ...
        trades = (
            db.query(Trade)
            .filter(
                Trade.status.in_(["CLOSED", "ONIAI_CLOSED"]),
