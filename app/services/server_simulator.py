...
from app.config import (
    ...
    BINANCE_WS_URL, BINANCE_REST_URL, BINANCE_WS_ENABLED,
)
...
class ServerSimulator:
    ...
    async def run(self):
        ...
        # Start Binance WebSocket (skip if disabled — e.g. Railway US servers get HTTP 451)
        if BINANCE_WS_ENABLED:
            await self.binance_ws.start()
        else:
            logger.info("Binance WS disabled (BINANCE_WS_ENABLED=false) — crypto via REST fallback")
        ...
        logger.info(
            ...
            f"Binance WS: {'ON' if BINANCE_WS_ENABLED else 'OFF (disabled)'}"
        )
