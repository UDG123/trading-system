...
class CryptoWSManager:
    ...
    async def _run_kraken(self):
        while self.running:
            try:
                ...
                self._kraken_connected = False
                logger.warning(f"Kraken WS error: {e} | reconnect in {self._kraken_reconnect_delay}s")
    ...
    async def _connect_kraken(self):
        ...
        async with websockets.connect(self.KRAKEN_WS, ping_interval=20, ping_timeout=10) as ws:
            ...
            async for message in ws:
                ...
                try:
                    ...
                    elif data.get("method") == "subscribe" and not data.get("success", True):
